...
def process_item(item, args):
    try:
        ...
        ligand = parse_sdf_file(os.path.join(args.source, item[1]), feat=False)
        ...
        ligand_fn = item[1]
        pocket_fn = item[1] + "_pocket%d.pdb" % args.radius
        ...
        return None, item[1], item[0], 0.0, None, None, None


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--source", type=str, default="./data/bindingmoad/all_data")
    parser.add_argument("--dest", type=str, default="./data/bindingmoad/pocket_10")
    ...
    for split in ["train", "val", "test"]:
        ...
        for item_pocket in tqdm(
            pool.imap_unordered(partial(process_item, args=args), index[split]),
            total=len(index[split]),
        ):
            pocket.append(item_pocket)
        # index_pocket = pool.map(partial(process_item, args=args), index)
        index_seq[split] = pocket
        pool.close()

    index_path = os.path.join(args.dest, "index_seq.pkl")
    with open(index_path, "wb") as f:
        pickle.dump(index_seq, f)
