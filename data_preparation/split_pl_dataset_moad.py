...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", type=str, default="./data/bindingmoad/pocket_10")
    parser.add_argument("--dest", type=str, default="./data/bindingmoad/split.pt")
    parser.add_argument(
        "--index", type=str, default="./data/bindingmoad/pocket_10/index_seq_split.pkl"
    )
    ...
    if args.index:
        ...
        for split in ["train", "test"]:
            ...
            for fn in index[split]:
                if fn[0] is not None and (fn[0] + fn[1]) in name_id_dict:
                    selected_ids[split].append(name_id_dict[fn[0] + fn[1]])
                else:
                    print(
                        f"Warning: data with PDB fn {fn[0]} and ligand fn {fn[1]} not found!"
                    )
        ...
        if args.val_num_pockets == -1:
            ...
            val_id, used_pdb = get_unique_pockets(
                dataset, raw_val_id, train_pdb, args.val_num_pockets
            )

        if args.test_num_pockets == -1:
            test_id = allowed_id[
                args.train + args.val : args.train + args.val + args.test
            ]
        else:
            raw_test_id = allowed_id[
                args.train + args.val : args.train + args.val + args.test
            ]
            test_id, used_pdb = get_unique_pockets(
                dataset, raw_test_id, used_pdb, args.test_num_pockets
            )
    ...
    print(
        "Train %d, Validation %d, Test %d." % (len(train_id), len(val_id), len(test_id))
    )
    print("Done.")
