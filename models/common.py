...
def compose_external_attention(batch_protein, batch_ligand, edit_protein_mask):
    num_graphs = batch_protein.max().item() + 1
    row, col = [], []
    protein_index, ligand_index = torch.arange(len(batch_protein)).to(
        batch_protein.device
    ), torch.arange(len(batch_ligand)).to(batch_protein.device)

    for i in range(num_graphs):
        mask_p, mask_l = (batch_protein == i), (batch_ligand == i)
        p_idx, q_idx = torch.cartesian_prod(
            protein_index[mask_p][edit_protein_mask[mask_p]], ligand_index[mask_l]
        ).chunk(2, dim=-1)
