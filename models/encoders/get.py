...
class GET(nn.Module):
    ...
    def __init__(
        ...
        if self.pre_norm:
            self.pre_layernorm = EquivariantLayerNorm(
                d_hidden, n_channel, n_rbf, cutoff, act_fn
            )
        ...
        for i in range(0, n_layers):
            ...
            self.add_module(
                f"layernorm0_{i}",
                EquivariantLayerNorm(d_hidden, n_channel, n_rbf, cutoff, act_fn),
            )
            self.add_module(
                f"ffn_{i}",
                EquivariantFFN(
    ...
    def forward(
        ...
        edges,
        edge_attr=None,
        cached_unit_edge_info=None,
    ):
        if cached_unit_edge_info is None:
            with torch.no_grad():
                cached_unit_edge_info = _unit_edges_from_block_edges(
        ...
        record_scale = torch.ones(
            (batch_size, n_channel, 1), dtype=torch.float, device=Z.device
        )
...
class GETLayer(nn.Module):
    ...
    def __init__(
        ...
        self.n_head = n_head
        assert (
            d_radial % self.n_head == 0
        ), f"d_radial not compatible with n_head ({d_radial} and {self.n_head})"
        assert (
            n_rbf % self.n_head == 0
        ), f"n_rbf not compatible with n_head ({n_rbf} and {self.n_head})"
        ...
        self.node_mlp = nn.Sequential(
            nn.Linear(d_radial, d_hidden), act_fn, nn.Linear(d_hidden, d_hidden), act_fn
        )
    ...
    def attention(self, H, Z, edges, edge_attr, cached_unit_edge_info):
        row, col = edges
        (unit_row, unit_col), (block_edge_id, unit_edge_src_start, unit_edge_src_id) = (
            cached_unit_edge_info
        )
        ...
        if self.n_rbf > 1:
            n_channel = D.shape[-1]
            D = self.rbf(D.view(-1)).view(
                D.shape[0], n_channel, self.n_head, -1
            )  # [Eu, n_channel, n_head, n_rbf / n_head]
            D = D.transpose(1, 2).reshape(
                D.shape[0], self.n_head, -1
            )  # [Eu, n_head, n_channel * n_rbf / n_head]
        else:
        ...
        if edge_attr is None:
            R_repr = torch.concat(
                [H_q, H_k, D], dim=-1
            )  # [Eu, n_head, (d_radial * 2 + n_channel * n_rbf) / n_head]
        else:
            edge_attr = self.edge_linear(edge_attr).view(
                edge_attr.shape[0], self.n_head, -1
            )
        ...
        alpha = scatter_softmax(R, unit_edge_src_id, dim=0).unsqueeze(
            -1
        )  # [Eu, n_head, 1], unit-level attention within block-level edges
        ...
        beta = self.block_att_linear(
            scatter_mean(R_repr, block_edge_id, dim=0)
        ).squeeze(
            -1
        )  # [Eb, n_head]
        beta = scatter_softmax(
            beta, row, dim=0
        )  # [Eb, n_head], block-level edge attention
        # beta = F.silu(beta)
        # for attention visualize
        # pickle.dump((alpha, beta, edges, (unit_row, unit_col)), open(f'./attention/{self.prefix}.pkl', 'wb'))
        beta = beta[block_edge_id[unit_edge_src_start]].unsqueeze(
            -1
        )  # [Em, n_head, 1], Em = \sum_{i, j \in E} n_i
    ...
    def invariant_update(self, H_v, H, alpha, beta, D, cached_unit_edge_info):
        ...
        H_v = self.unit_msg_mlp(
            torch.cat([H_v[unit_col], D], dim=-1)
        )  # [Eu, n_head, d_radial / n_head]

        H_agg = scatter_sum(
            alpha * H_v, unit_edge_src_id, dim=0
        )  # [Em, n_head, hidden_size / n_head]
        H_agg = H_agg.view(H_agg.shape[0], -1)  # [Em, hidden_size]
        H_agg = self.node_mlp(H_agg)  # [Em, hidden_size]
        H_agg = H_agg.view(
            H_agg.shape[0], self.n_head, -1
        )  # [Em, n_head, hidden_size / n_head]
        H_agg = scatter_sum(
            beta * H_agg, unit_agg_row, dim=0, dim_size=H.shape[0]
        )  # [N, n_head, hidden_size / n_head]
    ...
    def equivariant_update(self, H_v, Z, alpha, beta, D, dZ, cached_unit_edge_info):
        ...
        Z_agg = scatter_sum(
            ...
            unit_edge_src_id,
            dim=0,
        )  # [Em, n_head, n_channel, 3]
        Z_H_agg = scatter_sum(
            alpha * H_v, unit_edge_src_id, dim=0
        )  # [Em, n_head, d_radial / n_head]
        Z_H_agg = self.coord_mlp(Z_H_agg.view(Z_H_agg.shape[0], -1))  # [Em, d_radial]
        Z_H_agg = Z_H_agg.view(
            Z_H_agg.shape[0], self.n_head, -1
        )  # [Em, n_head, n_channel]
        Z_agg = scatter_sum(
            (beta * Z_H_agg).unsqueeze(-1) * Z_agg,
            unit_agg_row,
            dim=0,
            dim_size=Z.shape[0],
        )  # [N, n_head, n_channel, 3]
    ...
    def forward(
        self, H, Z, block_id, edges, edge_attr=None, cached_unit_edge_info=None
    ):
        """
        H: [N, hidden_size],
        Z: [N, n_channel, 3],
        ...
        alpha, beta, (D, R, dZ) = self.attention(
            H, Z, edges, edge_attr, cached_unit_edge_info
        )

        H_v = self.linear_v(
            H.view(H.shape[0], self.n_head, -1)
        )  # [N, n_head, d_radial / n_head]
...
class EquivariantLayerNorm(nn.Module):
    ...
    def __init__(
        self, d_hidden, n_channel, n_rbf=16, cutoff=7.0, act_fn=nn.SiLU()
    ) -> None:
        super().__init__()
    ...
    def forward(self, H, Z, block_id, batch_id):
        with torch.no_grad():
            ...
            unit_batch_id = batch_id[block_id]
            unit_axis_batch_id = (
                unit_batch_id.unsqueeze(-1).repeat(1, n_axis).flatten()
            )  # [N * 3]
        # H = self.layernorm(H)
        Z_c = scatter_mean(Z, unit_batch_id, dim=0)  # [bs, n_channel, 3]
        Z_c = Z_c[unit_batch_id]  # [N, n_channel, 3]
        ...
        rescale_rbf = self.rbf(rescale.view(-1)).view(
            rescale.shape[0], -1
        )  # [bs, n_channel * n_rbf]
...
if __name__ == "__main__":
    ...
    block_id = torch.tensor(
        [0, 0, 1, 1, 1, 1, 2, 2, 2, 3, 4, 4, 5, 6, 6, 6, 6, 7, 7], dtype=torch.long
    ).to(device)
