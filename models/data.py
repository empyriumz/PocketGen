    ...
    def __init__(self, jsonl_file, cdr_type, L_target):
        ...
        with open(jsonl_file) as f:
            ...
            for line in tqdm(all_lines):
                ...
                entry["binder_seq"] = "".join(
                    [entry["antibody_seq"][i] for i in surface.tolist()]
                )
                ...
                if (
                    len(entry["binder_coords"]) > 4
                    and len(entry["target_coords"]) > 4
                    and entry["antibody_cdr"].count("001") <= 1
                ):
...
def make_batch(batch):
    ...
    surface = (
        [b["binder_surface"] for b in batch],
        [b["target_surface"] for b in batch],
    )
