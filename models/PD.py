...
def interpolation_init(
    pred_X, residue_mask, backbone_pos, atom2residue, protein_atom_batch, residue_batch
):
    ...
    for i in range(num_protein):
        ...
        if (~residue_mask_i).sum() <= 2:
            ...
            residue_index = torch.arange(len(residue_mask_i)).to(
                protein_atom_batch.device
            )
            ...
            for k in range(len(residue_mask_i)):
                if residue_mask_i[k]:
                    ...
                    if k < front[0]:
                        pred_X[mask] = backbone_pos_i[front[0]] + (k - front[0]) / (
                            front[0] - front[1]
                        ) * (backbone_pos_i[front[0]] - backbone_pos_i[front[1]])
                    elif k > end[1]:
                        pred_X[mask] = backbone_pos_i[end[1]] + (k - end[1]) / (
                            end[1] - end[0]
                        ) * (backbone_pos_i[end[1]] - backbone_pos_i[end[0]])
                    else:
                        pred_X[mask] = (
                            (
                                (k - near[k][0]) * backbone_pos_i[near[k][1]]
                                + (near[k][1] - k) * backbone_pos_i[near[k][0]]
                            )
                            * 1
                            / (near[k][1] - near[k][0])
                        )
...
def interpolation_init_new(res_X, residue_mask, backbone_pos, residue_batch):
    ...
    backbone = torch.tensor(
        [[-0.525, 1.363, 0.0], [0.0, 0.0, 0.0], [1.526, 0.0, 0.0], [0.627, 1.062, 0.0]],
        device=res_X.device,
    )
    for i in range(num_protein):
        residue_mask_i = residue_mask[residue_batch == i]
                ...
                if residue_mask_i[k]:
                    ...
                    res_X[ind][:4] = (
                        ...
                        @ quaternion_to_matrix(
                            q=torch.randn(4, device=res_X.device)
                        ).t()
                    )
            offset += len(residue_mask_i)
...
class Pocket_Design_new(Module):
    def __init__(
        ...
        self.protein_atom_emb = nn.Embedding(
            protein_atom_feature_dim, int(config.hidden_channels / 2 - 8)
        )
        ...
        self.residue_embedding = nn.Embedding(
            21, int(config.hidden_channels / 2 - 16)
        ...
        if config.encoder.esm[:4] == "esm2":
            ...
            self.esmadapter = ESM2WithStructuralAdatper.from_pretrained(
                args=encoder_args, name=config.encoder.esm
            ).to(device)
        else:
            encoder_args = {
    ...
    def forward_(self, batch):
        ...
        res_X = interpolation_init_new(
            res_X,
            residue_mask,
            copy.deepcopy(batch["backbone_pos"]),
        ...
        for t in range(self.interpolate_steps):
            ...
            _, res_X, pred_res_type, pred_ligand = self.encoder(
                res_H,
                res_X.detach().clone(),
                res_S,
                batch["amino_acid_batch"],
                full_seq,
                pred_ligand.detach().clone(),
                ligand_feat,
                batch["ligand_mask"],
            ...
            loss_list[0] += 2 * self.huber_loss(
                res_X[residue_mask][atom_mask], label_X[residue_mask][atom_mask]
            ) + self.huber_loss(pred_ligand[ligand_mask], label_ligand[ligand_mask])
            loss_list[1] += self.pred_loss(
                pred_res_type,
    ...
    def init(self, batch):
        ...
        res_S = copy.deepcopy(batch["amino_acid_processed"])
        for k in range(
            len(batch["amino_acid"])
        ):  # init side chain atoms of masked residues
            if residue_mask[k]:
                pos = res_X[k]
                pos[4:] = pos[1].repeat(10, 1) + 0.1 * torch.randn(
                    10, 3, device=self.device
    ...
    def generate(self, batch, target_path="./generate"):
        ...
        for t in range(self.interpolate_steps):
            ...
            res_H, res_X, pred_ligand, ligand_feat, pred_res_type = self.encoder(
                res_H,
                res_X,
                res_S,
                res_batch,
        ...
        if self.write_pdb:
            ...
            to_sdf(
                pred_ligand,
                batch["ligand_element"].long(),
                batch["ligand_mask"].bool(),
                batch["ligand_batch"],
                batch["ligand_bond_type"].long(),
                batch["ligand_bond_index"].long(),
                batch["edge_batch"],
                self.generate_id,
                target_path,
            )
        ...
        if self.write_whole_pdb:
            self.generate_id1 = to_whole_pdb(
                res_X,
                res_S,
                batch["res_idx"],
                batch["amino_acid_batch"],
...
def random_mask(batch, device, mask=True):
    if mask:
        ...
        for k in range(num_residues):
            ...
            if k in index:
                ...
                if sidechain_size > 0:
                    pos_tmp.append(
                        batch["protein_pos"][mask][1:2].repeat(sidechain_size, 1)
                        + 0.1 * torch.randn(sidechain_size, 3, device=device)
                    )
        ...
        batch["protein_pos"], batch["protein_atom_feature"] = torch.cat(
            pos_tmp, dim=0
    ...
    batch["protein_atom_batch"] = torch.repeat_interleave(
...
def atom_feature(res_type, device):
    ...
    amino_acid = F.one_hot(res_type, num_classes=max_num_aa).repeat(
        NUM_ATOMS[res_type], 1
    )
...
def to_pdb(
    res_X, amino_acid, res_idx, res_batch, index, pocket_filename, target_path, original
):
        ...
        with open(path, "w") as f:
            ...
            for k in range(len(res_X_protein)):
                ...
                for i in range(NUM_ATOMS[amino_acid_protein[k]]):
                    ...
                    j3 = AA_NUMBER_NAME[amino_acid_protein[k].item()].ljust(
                        3
                    )  # resname#1s
            ...
            for k in range(len(residues)):
                if r10_mask[n, k + 1]:
                    ...
                    for i in range(NUM_ATOMS[amino_acid_protein[stored_res_count]]):
                        ...
                        j2 = str(atom_type[i]).center(4)  # atomname$#4s
                        j3 = AA_NUMBER_NAME[
                            amino_acid_protein[stored_res_count].item()
                        ].ljust(
                            3
                        )  # resname#1s
                        j4 = str(chain).rjust(1)  # Astring
                        j5 = str(res_idx_protein[stored_res_count].item()).rjust(
                            4
                        )  # resnum
                        j6 = str(
                            "%8.3f" % (float(res_X_protein[stored_res_count, i, 0]))
                        ).rjust(
                            8
                        )  # x
                        j7 = str(
                            "%8.3f" % (float(res_X_protein[stored_res_count, i, 1]))
                        ).rjust(
                            8
                        )  # y
                        j8 = str(
                            "%8.3f" % (float(res_X_protein[stored_res_count, i, 2]))
                        ).rjust(
                            8
                        )  # z\
                        j9 = str("%6.2f" % (1.00)).rjust(6)  # occ
...
def to_sdf(
    ...
    for l in range(num_ligand):
        ...
        edge_set = set()
        for k, (bond_type, (start_idx, end_idx)) in enumerate(
            zip(bond_types_protein, bond_index_protein)
        ):
            if (start_idx.item(), end_idx.item()) not in edge_set:
                ...
                mol.AddBond(
                    start_idx.item(), end_idx.item(), BOND_TYPE[bond_type.item()]
                )
...
class AminoAcidFeature(nn.Module):
    def __init__(self, backbone_only=False) -> None:
        ...
        for i in range(len(VOCAB)):
            if (
                ...
                residue_atom_pos.append(
                    [self.atom_pos_mask_idx for _ in range(n_channel)]
                )
            elif special_mask[i] == 1:
                # other special token (pad)
                residue_atom_type.append([self.atom_pad_idx for _ in range(n_channel)])
                residue_atom_pos.append(
                    [self.atom_pos_pad_idx for _ in range(n_channel)]
                )
            else:
                # normal amino acids
                ...
                if not backbone_only:
                    sidechain_atoms = VOCAB.get_sidechain_info(VOCAB.idx_to_symbol(i))
                    atom_type = atom_type + [
                        VOCAB.atom_to_idx(atom[0]) for atom in sidechain_atoms
                    ]
                    atom_pos = atom_pos + [
                        VOCAB.atom_pos_to_idx(atom[1]) for atom in sidechain_atoms
                    ]
                num_pad = n_channel - len(atom_type)
                residue_atom_type.append(
                    atom_type + [self.atom_pad_idx for _ in range(num_pad)]
                )
                residue_atom_pos.append(
                    atom_pos + [self.atom_pos_pad_idx for _ in range(num_pad)]
                )

        # mapping from residue to atom types and positions
        self.residue_atom_type = nn.parameter.Parameter(
            torch.tensor(residue_atom_type, dtype=torch.long), requires_grad=False
        )
        self.residue_atom_pos = nn.parameter.Parameter(
            torch.tensor(residue_atom_pos, dtype=torch.long), requires_grad=False
        )
        ...
        if not backbone_only:
                ...
                if special_mask[i] == 1:
                    ...
                    for src_atom in bond_atoms:
                        for dst_atom in bond_atoms[src_atom]:
                            bonds.append(
                                (atom2channel[src_atom], atom2channel[dst_atom])
                            )
            ...
            for i in range(len(VOCAB)):
                ...
                num_pad_chis, num_pad_bonds = (
                    max_num_chis - num_chis,
            ...
            self.sidechain_chi_angle_atoms = nn.parameter.Parameter(
                torch.tensor(sc_chi_atoms, dtype=torch.long), requires_grad=False
            )
            self.sidechain_chi_mask = nn.parameter.Parameter(
                torch.tensor(sc_chi_atoms_mask, dtype=torch.bool), requires_grad=False
            )
            self.sidechain_bonds = nn.parameter.Parameter(
                torch.tensor(sc_bonds, dtype=torch.long), requires_grad=False
            )
            self.sidechain_bonds_mask = nn.parameter.Parameter(
                torch.tensor(sc_bonds_mask, dtype=torch.bool), requires_grad=False
            )
    ...
    def _construct_residue_pos(self, S):
        ...
        glbl_node_idx = torch.nonzero(
            glbl_node_mask
        ).flatten()  # [batch_size * 3] (boa, boh, bol)
        shift = F.pad(
            glbl_node_idx[:-1] - glbl_node_idx[1:] + 1, (1, 0), value=1
        )  # [batch_size * 3]
...
class ProteinFeature(nn.Module):
    ...
    def _cal_angles(self, X, seg_id):
        ...
        cosA1 = ((-u_0) * u_1).sum(
            -1
        )  # [N*3 - 2], (C_{n-1}-N, N-CA), (N-CA, CA-C), (CA-C, C-N_{n+1})
        same_chain_mask = sequential_and(
            seg_id_atom[:-2] == seg_id_atom[1:-1], seg_id_atom[1:-1] == seg_id_atom[2:]
        )
    ...
    def coord_loss(self, pred_X, true_X, batch_id, atom_mask, reference=None):
        ...
        xloss = (
            F.smooth_l1_loss(pred_X[atom_mask], true_X[atom_mask], reduction="sum")
            / atom_mask.sum()
        )  # atom-level loss
        bb_rmsd = torch.sqrt(
            ((pred_X[:, :4] - true_X[:, :4]) ** 2).sum(-1).mean(-1)
        )  # [N]
