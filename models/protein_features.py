...
class ProteinFeatures(nn.Module):
    ...
    def _orientations_coarse(self, X, edge_index, residue_batch, eps=1e-6):
        ...
        dX = X[col] - X[row]
        dU = torch.matmul(O.reshape(O.shape[0], 3, 3)[col], dX.unsqueeze(-1)).squeeze(
            -1
        )
    ...
    def forward(self, pos_ligand_coarse, edit_residue, X, S_id, batch):
        ...
        col = torch.cat(
            [
                torch.ones(edit_residue[batch == s].sum(), dtype=torch.long) * s
                for s in range(batch.max().item() + 1)
            ]
        ).to(X.device)
        ...
        E_new = torch.cat(
            [
                torch.zeros(len(row), 16, device=X.device),
                RBF,
                torch.zeros(len(row), 7, device=X.device),
            ],
            -1,
        )
        ...
        return (
            ...
            edge_index,
            edge_length,
            torch.cat([row.unsqueeze(0), (col + len(X)).unsqueeze(0)], 0),
            E_new,
        )
