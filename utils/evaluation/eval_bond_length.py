...
def get_distribution(
    distances: Sequence[float], bins=eval_bond_length_config.DISTANCE_BINS
) -> np.ndarray:
...
def get_bond_length_profile(
    bond_lengths: Sequence[BondLengthData],
) -> BondLengthProfile:
...
def eval_bond_length_profile(
    bond_length_profile: BondLengthProfile,
    ...
    for (
        bond_type,
        gt_distribution,
    ) in eval_bond_length_config.EMPIRICAL_DISTRIBUTIONS.items():
        if bond_type not in bond_length_profile:
            metrics[f"JSD_{_bond_type_str(bond_type)}"] = None
        else:
            metrics[f"JSD_{_bond_type_str(bond_type)}"] = (
                sci_spatial.distance.jensenshannon(
...
def eval_pair_length_profile(pair_length_profile):
    ...
    for (
        k,
        gt_distribution,
    ) in eval_bond_length_config.PAIR_EMPIRICAL_DISTRIBUTIONS.items():
        if k not in pair_length_profile:
            metrics[f"JSD_{k}"] = None
        else:
            metrics[f"JSD_{k}"] = sci_spatial.distance.jensenshannon(
                gt_distribution, pair_length_profile[k]
            )
    return metrics
...
def plot_distance_hist(pair_length_profile, metrics=None, save_path=None):
    ...
    for idx, (k, gt_distribution) in enumerate(
        eval_bond_length_config.PAIR_EMPIRICAL_DISTRIBUTIONS.items()
    ):
