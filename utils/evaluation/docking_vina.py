...
class PrepLig:
    def __init__(self, input_mol, mol_format):
        ...
        self.preparator = MoleculePreparation()
    ...
    def get_pdbqt(self, lig_pdbqt=None):
        ...
        # Prepare the RDKit molecule
        setups = self.preparator.prepare(rdkit_mol)

        if lig_pdbqt is not None:
            pdbqt_string, is_ok, err_msg = PDBQTWriterLegacy.write_string(setups[0])
            if not is_ok:
                raise RuntimeError(f"Cannot generate PDBQT: {err_msg}")
    ...
    # @supress_stdout
    # def get_pdbqt(self, lig_pdbqt=None):
    #     setups = self.preparator.prepare(self.ob_mol.OBMol)
    #     if lig_pdbqt is not None:
    #         pdbqt_string, is_ok, err_msg = PDBQTWriterLegacy.write_string(setups[0])
    #         if not is_ok:
    #             raise RuntimeError(f'Cannot generate PDBQT: {err_msg}')
...
class PrepProt:
    ...
    def addH(self, prot_pqr):  # call pdb2pqr
        ...
        try:
            ...
            print(f"Error running pdb2pqr: {e.stderr}")
            raise
    ...
    def get_pdbqt(self, prot_pdbqt):
        ...
        try:
            ...
            print(f"Error preparing receptor: {e.stderr}")
            raise
