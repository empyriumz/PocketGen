...
if __name__ == "__main__":
    ...
    def train(it, loss_list, metric_list):
        ...
        pred_loss = model.pred_loss(
            pred_res_type,
            model.standard2alphabet[batch["amino_acid"][residue_mask] - 1],
        )
        struct_loss = 2 * model.proteinloss.structure_loss(
            res_X[residue_mask],
        ...
        if it % freq == 0:
            ...
            logger.info(
                "[Train] Iter %d | Loss %.6f | Loss(huber) %.6f | Loss(pred) %.6f | Loss(bond & andgle) %.6f | AAR %.6f | RMSD %.6f "
                "|Orig_grad_norm %.6f"
                % (
                    it,
                    total_loss,
                    loss_list[0].item() / freq,
                    loss_list[1].item() / freq,
                    loss_list[2] / freq,
                    metric_list[0].item() / freq,
                    metric_list[1].item() / freq,
                    orig_grad_norm,
                )
            )
            wandb.log(
                {
                    "loss": total_loss,
                    "Loss(huber)": loss_list[0].item() / freq,
                    "Loss(pred)": loss_list[1].item() / freq,
                    "aar": metric_list[0].item() / freq,
                    "rmsd": metric_list[1].item() / freq,
                }
            )
    ...
    def validate(it):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(val_loader, desc="Validate"):
                ...
                loss = model.huber_loss(
                    res_X[residue_mask][atom_mask], label_X[residue_mask][atom_mask]
                ) + model.huber_loss(ligand_pos[ligand_mask], label_ligand[ligand_mask])
                loss += model.pred_loss(
                    pred_res_type,
                    model.standard2alphabet[batch["amino_acid"][residue_mask] - 1],
                )
                loss += 2 * model.proteinloss.structure_loss(
                    res_X[residue_mask],
                    label_X[residue_mask],
                    batch["amino_acid"][residue_mask] - 1,
                    batch["res_idx"][residue_mask],
        ...
        writer.flush()
        wandb.log({"val_loss": avg_loss, "val_aar": aar, "val_rmsd": rmsd})
        return avg_loss
